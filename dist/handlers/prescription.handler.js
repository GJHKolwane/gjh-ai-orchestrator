      ...
      * - Check regional connectivity
       * - Use ONLINE mode if reachable
        * - Fallback to OFFLINE_AUTONOMOUS if not
         */
export async function prescriptionHandler(req, res) {
    const { consultationId, facilityId, medication, quantity, prescriberId } = req.body;
    ...
    if (!regionalOnline) {
        ...
        const offlineDecision = evaluateOfflineStock(medication, facilityId, quantity);
    ...
    try {
        ...
        const response = await axios.post(`${gatewayUrl}/medicine`, {
